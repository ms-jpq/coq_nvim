...
T.test("future resolves synchronously", function()
  local resolve, await = async.future()
  resolve "woof"
  T.eq(await(), "woof")
end)

T.test("wrap returns callback args", function()
...
T.test("wrap forwards multiple callback values", function()
...
T.test("thunk defers execution", function()
  local ran = false
...
T.test("errors propagate from run", function()
...
T.test("sleep yields for the requested duration", function()
