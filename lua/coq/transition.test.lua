...
local find = function(opts)
  return vim.iter(transition.audit_findings(opts)):totable()
end
...
T.describe({ "transition.audit_findings" }, function(test)
  ...
  test({ "a removed root option is flagged" }, function()
    T.eq(find { xdg = true }, { { kind = "option", path = "xdg" } })
  end)

  test({ "a removed option deep under display.pum is flagged with its hint" }, function()
    T.eq(find { display = { pum = { y_max_len = 16 } } }, {
      {
        kind = "option",
  ...
  test({ "a removed option with no hint yields no hint field" }, function()
    T.eq(find { display = { pum = { fast_close = true } } }, {
      { kind = "option", path = "display.pum.fast_close" },
    })
  end)

  test({ "a removed client bucket is flagged with its hint" }, function()
    T.eq(find { clients = { tabnine = { enabled = true } } }, {
  ...
  test({ "a per-client removed knob is flagged with full path" }, function()
    T.eq(find { clients = { lsp = { max_pulls = 188 } } }, {
      { kind = "option", path = "clients.lsp.max_pulls" },
    })
  end)

  test({ "a client-specific removed knob is flagged" }, function()
    T.eq(find { clients = { tree_sitter = { slow_threshold = 0.5 } } }, {
      { kind = "option", path = "clients.tree_sitter.slow_threshold" },
    })
  end)
  ...
  test({ "wired field next to a removed field reports only the removed one" }, function()
    T.eq(
      ...
      { { kind = "option", path = "clients.buffers.match_syms" } }
    )
  end)
  ...
  test({ "multiple removed paths surface together" }, function()
    T.eq(
      ...
      sorted {
        { kind = "option", path = "clients.lsp.always_wait" },
        { kind = "client", path = "tabnine", hint = "use the TabNine plugin directly." },
        { kind = "option", path = "xdg" },
      }
    )
  end)
