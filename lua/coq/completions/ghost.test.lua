...
T.describe({ "ghost.show" }, function(test)
  ...
  test({ "inline leaves the suffix to neovim" }, function()
    local buf, ctx = mk_ctx({ "./ft  .lua" }, 1, 4)
    ghost.show(ctx, item_of "ftplugin")
    T.eq(extmark_opts(ghost_cfg, buf, ctx.pos[2]).virt_text, { { "plugin", "Comment" } })

    vim.api.nvim_buf_set_lines(buf, 0, 1, true, { "./ft  .py" })
    T.eq(extmark_opts(ghost_cfg, buf, ctx.pos[2]).virt_text, { { "plugin", "Comment" } })

    local spaced_buf, spaced_ctx = mk_ctx({ "./ft  .lua" }, 1, 4)
...
T.describe({ "ghost.show range-driven overlay" }, function(test)
  ...
  test({ "non-empty range → overlay (mirrors vim.lsp.inline_completion)" }, function()
    local buf, ctx = mk_ctx({ "fiXYZ  .lua" }, 1, 2)
    ghost.show(ctx, item_with_range("fidoXYZ", 0, 5))
    local d = mark_details(buf, ctx)
    assert(d, "expected an extmark")
    T.eq(d.virt_text_pos, "overlay")
    T.eq(d.virt_text[1][1], "do")
  end)
  ...
  test({ "tab inside ranged candidate expands; stays overlay" }, function()
    local buf, ctx = mk_ctx({ "xxxxx" }, 1, 0)
    ghost.show(ctx, item_with_range("\tbodyxxxxx", 0, 5))
    local d = mark_details(buf, ctx)
    assert(d, "expected an extmark")
    T.eq(d.virt_text_pos, "overlay")
