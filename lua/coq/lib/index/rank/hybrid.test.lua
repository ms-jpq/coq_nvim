...
T.describe({ "rank.hybrid._STAGES" }, function(test)
  ...
  test({ "exact: byte-equal returns T_EXACT" }, function()
    T.eq(hybrid._STAGES.exact(probe("dog", "dog")), 2 ^ 30)
  end)

  test({ "exact: case-fold-equal returns nil (byte-exact only)" }, function()
    T.eq(hybrid._STAGES.exact(probe("dog", "Dog")), nil)
  end)

  test({ "exact: any other input returns nil" }, function()
    T.eq(hybrid._STAGES.exact(probe("dog", "doggo")), nil)
    T.eq(hybrid._STAGES.exact(probe("dog", "labrador")), nil)
  end)

  -- prefix ---------------------------------------------------------------

  test({ "prefix: smart-case prefix returns a positive score" }, function()
    assert(hybrid._STAGES.prefix(probe("lab", "labrador")) ~= nil)
    assert(hybrid._STAGES.prefix(probe("lab", "Labrador")) ~= nil)
  end)
  ...
  test({ "prefix: does NOT short-circuit on exact equality" }, function()
    ...
    local got = hybrid._STAGES.prefix(probe("dog", "dog"))
  ...
  test({ "prefix: non-prefix returns nil" }, function()
    T.eq(hybrid._STAGES.prefix(probe("lab", "global_lab")), nil)
  end)

  test({ "prefix: smart-case rejection returns nil" }, function()
    T.eq(hybrid._STAGES.prefix(probe("Lab", "labrador")), nil)
  end)

  -- camel ----------------------------------------------------------------

  test({ "camel: matching initialism returns a positive score" }, function()
    assert(hybrid._STAGES.camel(probe("agn", "AnimGraphNode")) ~= nil)
    assert(hybrid._STAGES.camel(probe("agn", "anim_graph_node")) ~= nil)
  end)

  test({ "camel: non-initialism returns nil" }, function()
    T.eq(hybrid._STAGES.camel(probe("agn", "BatchAgnSelector")), nil)
    T.eq(hybrid._STAGES.camel(probe("xyz", "AnimGraphNode")), nil)
  end)

  test({ "camel: shorter haystack wins on tiebreak" }, function()
    local short = assert(hybrid._STAGES.camel(probe("agn", "AnimGraphNode")))
    local long = assert(hybrid._STAGES.camel(probe("agn", "AnimationGraphNode")))
  ...
  test({ "fuzzy: subsequence match returns a positive score" }, function()
    local got = hybrid._STAGES.fuzzy(probe("lbr", "labrador"))
    assert(got ~= nil and got > 0)
  end)

  test({ "fuzzy: non-subsequence returns nil" }, function()
    T.eq(hybrid._STAGES.fuzzy(probe("xyz", "labrador")), nil)
  end)

  test({ "fuzzy: smart-case rejection propagates as nil" }, function()
    T.eq(hybrid._STAGES.fuzzy(probe("Lab", "labrador")), nil)
  end)

  -- pipeline ordering ----------------------------------------------------

  test({ "score: returns try_exact result when byte-equal" }, function()
    T.eq(hybrid.score("dog", "dog"), hybrid._STAGES.exact(probe("dog", "dog")))
  end)

  test({ "score: returns try_prefix result when smart-case prefix matches" }, function()
    T.eq(hybrid.score("lab", "labrador"), hybrid._STAGES.prefix(probe("lab", "labrador")))
  end)

  test({ "score: returns try_camel result for initialism non-prefix" }, function()
    T.eq(hybrid.score("agn", "AnimGraphNode"), hybrid._STAGES.camel(probe("agn", "AnimGraphNode")))
  end)

  test({ "score: returns try_fuzzy result for plain subsequence" }, function()
    T.eq(hybrid.score("lbr", "labrador"), hybrid._STAGES.fuzzy(probe("lbr", "labrador")))
  end)
end)
