...
async def _load_paths(
    ...
    def cont() -> None:
        for p in paths:
            ...
            with suppress(OSError):
                for path in walk(user):
                    if path.suffix in {".snip"} and path.is_file():
                        user_defined[path] = path.stat().st_mtime
...
# async def _compile_user(path: Path, mtime: float) -> LoadedSnips:
#     loaeded = await run_in_executor(
#         lambda: load_direct(lsp={}, neosnippet={path: mtime}, ultisnip={})
#     )
