...
CREATE TABLE IF NOT EXISTS
  sources (name TEXT NOT NULL PRIMARY KEY) WITHOUT ROWID;

CREATE TABLE IF NOT EXISTS
  batches (rowid BLOB NOT NULL PRIMARY KEY) WITHOUT rowid;
...
FROM
  instances
  LEFT JOIN instance_stats ON instance_stats.instance_id = instances.rowid;

CREATE VIEW IF NOT EXISTS
  stats_quantiles_view AS
SELECT
  source AS source,
...
CREATE VIEW IF NOT EXISTS
  stats_inserted_view AS
SELECT
  instances.source_id AS source,
  COUNT(inserted.rowid) AS inserted
FROM
  instances
  LEFT JOIN inserted ON inserted.instance_id = instances.rowid
GROUP BY
  instances.source_id;
...
FROM
  sources
  LEFT JOIN stats_quantiles_view ON stats_quantiles_view.source = sources.name
  LEFT JOIN stats_inserted_view ON stats_inserted_view.source = sources.name;
