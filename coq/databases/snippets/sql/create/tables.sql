...
CREATE TABLE IF NOT EXISTS
  filetypes (filetype TEXT NOT NULL PRIMARY KEY) WITHOUT ROWID;
...
WITH RECURSIVE
  all_exts AS (
    ...
    SELECT
      ...
      e2.dest
    FROM
      uniq_extensions_view AS e2
      JOIN all_exts ON all_exts.dest = e2.src
  )
SELECT
...
FROM
  snippets
  JOIN matches ON matches.snippet_id = snippets.rowid
  JOIN extensions_view ON snippets.filetype = extensions_view.dest
WHERE
  matches.word <> ''
  AND snippets.content <> '';
