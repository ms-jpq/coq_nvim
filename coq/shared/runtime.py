...
from asyncio import Condition, Task, as_completed, gather, sleep, wait
...
class Supervisor:
    ...
    async def collect(self, context: Context) -> Sequence[Metric]:
        with timeit("COLLECTED -- **ALL**"):
            ...
            timeout = (
                self.limits.manual_timeout if context.manual else self.limits.timeout
            )
                ...
                with timeit(f"WORKER -- {assoc.short_name}"):
                    ...
                    try:
                        async for completions in worker.work(context):
                            for comps in chunk(completions, n=self.options.max_results):
                                if not done:
